...
if (!$task) {
    Session::setFlash('error', 'Không tìm thấy công việc hoặc bạn không có quyền chỉnh sửa.');
    header('Location: ../index.php');
    exit;
}
...
// Lấy tags hiện tại của task
$currentTags = $taskModel->getTaskTags($taskId);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Cập nhật task
    if (empty($errors)) {
        if ($taskModel->update($taskId, $userId, $title, $description ?: null, $dueDate ?: null, $dueTime ?: null, $status, $categoryId ?: null, $priority, $isImportant)) {
            // Cập nhật tags
            $taskModel->attachTags($taskId, $tagIds);
            
            Session::setFlash('success', 'Cập nhật công việc thành công!');
            header('Location: ../index.php');
            exit;
        } else {
...
<head>
    ...
    <title>Chỉnh sửa công việc - <?php echo APP_NAME; ?></title>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between h-16">
                <div class="flex items-center">
                    ...
                    <h1 class="text-2xl font-bold text-gray-900">Chỉnh sửa công việc</h1>
                </div>
            ...
            <form method="POST" action="" class="space-y-6">
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="4"
                              ...
                              placeholder="Nhập mô tả chi tiết (tùy chọn)"><?php echo htmlspecialchars($description); ?></textarea>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select id="priority" name="priority"
                            ...
                            <option value="medium" <?php echo $priority === 'medium' ? 'selected' : ''; ?>>Trung bình</option>
                            <option value="high" <?php echo $priority === 'high' ? 'selected' : ''; ?>>Cao</option>
                            <option value="urgent" <?php echo $priority === 'urgent' ? 'selected' : ''; ?>>Khẩn cấp</option>
                        </select>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <select id="status" name="status"
                        ...
                        <option value="in_progress" <?php echo $status === 'in_progress' ? 'selected' : ''; ?>>Đang làm</option>
